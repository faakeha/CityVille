...
const { verifyToken, verifyTokenAndAuth, verifyToken2 } = require('./verifyToken');
...
router.get("/Services", async (req,res)=>{
    //const user1 =  await User.findOne({_id:req.user.id});
        if(!req?.query?.id) { //user
            const all_services = await Service.find({approve_status : "Approved"},{is_paused : false});
...
//Update service
router.put("/updateService/:id", verifyToken, async (req, res) => { //service id will be given
...
//update appointment
router.put("/updateApp/:id", verifyToken, async(req,res)=>{ //appointment id
        ...
        if(user1.user_role == 1){
            ...
            if(user_app.to_user_id != req.user.id){
                ...
                if(!req.body.status){
                    ...
                    if(user_app.status == "Pending" && (req.body.status == "Approved" || req.body.status =="Rejected")){
                        const updatedApp = await Appointment.findByIdAndUpdate(
                            req.params.id,
                            {
                              $set: req.body,
                    ...
                    else if(user_app.status == "Approved" && (req.body.status == "Cancelled" || req.body.status == "Completed")){
                        const updatedApp = await Appointment.findByIdAndUpdate(
                            req.params.id,
                            {
                              $set: req.body,
...
router.put("/updateReport/:id", verifyToken, async (req, res) => {
	...
	if (user1.user_role == 1) {
        if(!req.body.report_status){
            ...
            if(report.report_status == "Pending" && (req.body.report_status == "Approved" || req.body.report_status == "Rejected")){
